...
def luceneFuzzySearchPercentage(list, text, threshold):
    res = []
    for s in list:
        t = []

        dist = levenshteinDistance(s, text)
        l = float(max(len(s), len(text)))
        score = (l - dist)/l
...
threshold = 30.01
l = luceneFuzzySearchPercentage(city, 'akro', threshold)
for i in l:
    print(i)
